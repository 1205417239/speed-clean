    ...
    while (cur) {
      for (uint j = 0; j < cur->rebindings_nel; j++) {
        if (symbol_name_longer_than_1 &&
          ...
          mprotect((void *)((uintptr_t)indirect_symbol_bindings + i * sizeof(void *)),
                   sizeof(void *),
...
static void rebind_symbols_for_image(
    struct rebindings_entry *rebindings,
...
int rebind_symbols_image(void *header,
  ...
  rebind_symbols_for_image(rebindings_head, (const mach_header_t *)header, slide);
...
int rebind_symbols(struct rebinding rebindings[], size_t rebindings_nel) {
  ...
  if (!_rebindings_head->next) {
    ...
    for (uint32_t i = 0; i < c; i++) {
      _rebind_symbols_for_image((const mach_header_t *)_dyld_get_image_header(i),
